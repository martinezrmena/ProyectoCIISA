...
{
    public class ctrlCarga_DetalleReses
    {
        public ctrlCargaMenu controlador { get; set; }
        public string pcodCliente = string.Empty;

        internal ctrlCarga_DetalleReses(ctrlCargaMenu pcontrolador)
        {
            controlador = pcontrolador;
        }

        public async void informacionDetalleResesRecargar()
        {
            ...
            if (await _testConnectionSROL.testConnectionBoolean(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL), true))
            {
                ...
                string _memoryStream = ServicioUpload.Get_cargaDetalleReses(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));

                if (!_memoryStream.Equals(TypeEvents._errorWS))
                {
                    _log.addLineSuccessWSDownload(_textBox, TablesROL._DetalleReses);

                    var _memoryCompress = DependencyService.Get<IMC_HH>();

                    DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionDetalleReses", TypeTransaction._upload, _memoryStream, TablesROL._DetalleReses);

                    _guiCarga.actualizarGUIdesdeHilo_45_porciento();

                    if (_dt.Rows == null)
                    {
                        _log.addErrorLineDataTableNull(_textBox, TablesROL._DetalleReses);
                    ...
                    else
                    {
                        if (_dt.Rows.Count != 0)
                        {
                            ...
                            {
                                Carga_ManagerDetalleReses _manager = new Carga_ManagerDetalleReses();

                                StringBuilder _sb = _manager.insertTablaDetalleReses(_fila);
                            ...
                            _log.registrationNumberInserts(_textBox, _inserciones, _dt.Rows.Count, TablesROL._DetalleReses);

                            _guiCarga.actualizarGUIdesdeHilo_85_porciento();
                        }
                        else
                        {
                            _log.addAlertLine(_textBox, "la consulta a la tabla " + TablesROL._DetalleReses + " no devolvio resultados.");

...
{
    public class ctrlCarga_MensajeFactura
    {
        public ctrlCargaMenu controlador { get; set; }
        public string pcodCliente = string.Empty;

        internal ctrlCarga_MensajeFactura(ctrlCargaMenu pcontrolador)
        {
            controlador = pcontrolador;
        }

        public async void informacionMensajeFacturaRecargar()
        {
            ...
            if (await _testConnectionSROL.testConnectionBoolean(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL), true))
            {
                ...
                string _memoryStream = ServicioUpload.Get_cargaMensajeFactura(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));

                if (!_memoryStream.Equals(TypeEvents._errorWS))
                {
                    _log.addLineSuccessWSDownload(_textBox, TablesROL._MensajeFactura);

                    var _memoryCompress = DependencyService.Get<IMC_HH>();

                    DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionMensajeFactura", TypeTransaction._upload, _memoryStream, TablesROL._MensajeFactura);

                    _guiCarga.actualizarGUIdesdeHilo_45_porciento();

                    if (_dt.Rows == null)
                    {
                        _log.addErrorLineDataTableNull(_textBox, TablesROL._MensajeFactura);
                    ...
                    else
                    {
                        if (_dt.Rows.Count != 0)
                        {
                            ...
                            {
                                Carga_ManagerMensajeFactura _manager = new Carga_ManagerMensajeFactura();

                                StringBuilder _sb = _manager.insertTablaMensajeFactura(_fila);
                            ...
                            _log.registrationNumberInserts(_textBox, _inserciones, _dt.Rows.Count, TablesROL._MensajeFactura);

                            _guiCarga.actualizarGUIdesdeHilo_85_porciento();
                        }
                        else
                        {
                            _log.addAlertLine(_textBox, "la consulta a la tabla " + TablesROL._MensajeFactura + " no devolvio resultados.");

...
{
    public class ctrlCarga_precioProducto
    {
        private ctrlCargaMenu controlador { get; set; }

        internal ctrlCarga_precioProducto(ctrlCargaMenu pcontrolador)
        {
            controlador = pcontrolador;
        }

        public async void informacionPrecioProductoRecargar()
        {
            ...
            if (await _testConnectionSROL.testConnectionBoolean(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL), true))
            {
                ...
                string _memoryStream = ServicioUpload.Get_cargaPrecioProducto(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));

                if (!_memoryStream.Equals(TypeEvents._errorWS))
                {
                    _log.addLineSuccessWSDownload(_textBox, TablesROL._precioProducto);

                    var _memoryCompress = DependencyService.Get<IMC_HH>();

                    DataTable _dt = _memoryCompress.Unzip_HH_DataTable("informacionPrecioProductoRecargar", TypeTransaction._upload, _memoryStream, TablesROL._precioProducto);

                    _guiCarga.actualizarGUIdesdeHilo_45_porciento();

                    if (_dt.Rows == null)
                    {
                        _log.addErrorLineDataTableNull(_textBox, TablesROL._precioProducto);
                    ...
                    else
                    {
                        if (_dt.Rows.Count != 0)
                        {
                            ...
                            {
                                Carga_ManagerPrecioProducto _manager = new Carga_ManagerPrecioProducto();

                                StringBuilder _sb = _manager.insertTablaPrecioProducto(_fila);
                            ...
                            _log.registrationNumberInserts(_textBox, _inserciones, _dt.Rows.Count, TablesROL._precioProducto);

                            _guiCarga.actualizarGUIdesdeHilo_85_porciento();
                        }
                        else
                        {
                            _log.addAlertLine(_textBox, "la consulta a la tabla " + TablesROL._precioProducto + " no devolvio resultados.");

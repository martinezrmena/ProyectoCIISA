...
{
    public class ctrlCarga_DatosTablaSistema
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaSistema(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionSistema(
            string ptable,
            Editor ptextBox,
            Label plabel,
            Log plog
            )
        {
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionSistema", TypeTransaction._upload, _memoryStream, TablesROL._sistema);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaSistema(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }
    }

...
{
    public class ctrlCarga_DatosTablaDetalleReses
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaDetalleReses(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        public async Task cargaInformacionDetalleReses(
            string ptable,
            Editor ptextBox,
            Label plabel,
            Log plog
            )
        {
            ...
            string _memoryStream = servicio.Get_cargaDetalleReses(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaDetalleReses(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }

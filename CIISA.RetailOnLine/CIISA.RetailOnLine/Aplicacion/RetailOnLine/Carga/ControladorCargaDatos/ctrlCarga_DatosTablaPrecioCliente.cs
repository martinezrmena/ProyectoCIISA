...
{
    public class ctrlCarga_DatosTablaPrecioCliente
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaPrecioCliente(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionPrecioCliente(string ptable,Editor ptextBox,Label plabel,Log plog)
        {
            var servicio = DependencyService.Get<IService_WebServiceUpload>();

            string _memoryStream = servicio.Get_cargaPrecioCliente(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionPrecioCliente", TypeTransaction._upload, _memoryStream, TablesROL._precioCliente);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaPrecioCliente(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }

...
{
    public class ctrlCarga_DatosTablaTipoTransaccion
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaTipoTransaccion(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionTipoTransaccion(
            string ptable,
            Editor ptextBox,
            Label plabel,
            Log plog
            )
        {
            ...
            string _memoryStream = servicio.Get_cargaTipoTransaccion(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionTipoTransaccion", TypeTransaction._upload, _memoryStream, TablesROL._tipoTransaccion);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaTipoTransaccion(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }

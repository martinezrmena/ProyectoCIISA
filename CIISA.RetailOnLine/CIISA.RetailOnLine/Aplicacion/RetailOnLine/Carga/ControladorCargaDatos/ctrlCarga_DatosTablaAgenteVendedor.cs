...
{
    public class ctrlCarga_DatosTablaAgenteVendedor
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaAgenteVendedor(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionAgenteVendedor(string ptable,Editor ptextBox,Label plabel,Log plog)
        {
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionAgenteVendedor", TypeTransaction._upload, _memoryStream, TablesROL._agenteVendedor);

                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox,plog))
                {
                    OperationSQL.deleteTableFeedbackTextBox(ptable,ptextBox,plog);

                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaAgenteVendedor(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }
    }

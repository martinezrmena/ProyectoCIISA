...
{
    public class ctrlCarga_DatosTablaProducto
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaProducto(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionProducto(
            string ptable,
            Editor ptextBox,
            Label plabel,
            Log plog
            )
        {
            ...
            string _memoryStream = servicio.Get_cargaProducto(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionProducto", TypeTransaction._upload, _memoryStream, TablesROL._producto);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaProducto(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }

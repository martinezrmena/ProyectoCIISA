...
{
    public class ctrlCarga_DatosTablaPrecioProducto
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaPrecioProducto(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionPrecioProducto(string ptable,Editor ptextBox,Label plabel,Log plog)
        {
            var servicio = DependencyService.Get<IService_WebServiceUpload>();

            string _memoryStream = servicio.Get_cargaPrecioProducto(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionPrecioProducto", TypeTransaction._upload, _memoryStream, TablesROL._precioProducto);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaPrecioProducto(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }

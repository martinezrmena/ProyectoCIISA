...
{
    public class ctrlCarga_DatosTablaImpresora
    {
        private Carga_ManagerDatosTabla v_managerDatosTabla { get; set; }

        public ctrlCarga_DatosTablaImpresora(Carga_ManagerDatosTabla pmanagerDatosTabla)
        {
            v_managerDatosTabla = pmanagerDatosTabla;
        }

        internal async Task cargaInformacionImpresora(
            string ptable,
            Editor ptextBox,
            Label plabel,
            Log plog
            )
        {
            ...
            string _memoryStream = servicio.Get_cargaImpresora(Sistema.establecerObjetoSystemCIISA(VersionROL._versionSROL));
            ...
            if (!_memoryStream.Equals(TypeEvents._errorWS))
            {
                ...
                DataTable _dt = _memoryCompress.Unzip_HH_DataTable("cargaInformacionImpresora", TypeTransaction._upload, _memoryStream, TablesROL._impresora);
                ...
                if (DataTableValidate.validateDataTable(_dt, ptable, ptextBox, plog))
                {
                    ...
                    await v_managerDatosTabla.v_helperCargaDatosTabla.cargaTablaImpresora(ptable, _dt, ptextBox, plabel, plog);
                }
            }
        }
